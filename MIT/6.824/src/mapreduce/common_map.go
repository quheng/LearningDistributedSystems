...
func doMap(
	...
	fileWriters := make([]*json.Encoder, nReduce)
	for r := 0; r < nReduce; r++ {
		...
		fileWriter := json.NewEncoder(file)
		fileWriters[r] = fileWriter
		files[r] = file
	}

	for _, keyValue := range keyValues {
		err := fileWriters[ihash(keyValue.Key)%nReduce].Encode(&keyValue)
		if err != nil {
			panic(err)
		}
	}
